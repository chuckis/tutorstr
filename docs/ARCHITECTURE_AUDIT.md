...
| --- | --- | --- | --- | --- |
| `frontend/src/hooks/useBookings.ts:2` | Closed on 2026-05-16: booking event to domain mapping now flows through the Nostr booking repository adapter instead of importing `bookingFromNostr` directly in the hook. | Dependency Rule; Hook Boundaries | Completed via `frontend/src/adapters/nostr/bookingRepository.ts` exporting `mapNostrBookings(...)`, with `useBookings.ts` reduced to adapter-backed mapping plus hook orchestration. | S |
